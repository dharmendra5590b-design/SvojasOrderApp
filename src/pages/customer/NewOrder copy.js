...
const [quality, setQuality] = useState([]);
  ...
  const load = async () => {
    try { const { data:order } = await api.get('https://localhost:8081/api/order/GetListCustomerOrder',{employee_ID:0}); 
  ...
  const handleSubmit = async (ev) => {
    ...
    const {data} = await api.post('https://localhost:8081/api/order/SaveOrder', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
